  ...
  return nowPlayingMovies.length > 0 ?(
    <>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex itmes-center gap-2 border border-gray-600
        px-3 py-2 rounded-md'>
          <p className='text-gray-400 text-sm'>{currency}</p>
          <input type="number" min={0} value={showPrice} onChange={
            (e)=> setShowPrice(e.target.value)} placeholder='Enter show price' className='outline-none'/>
        </div>
      </div>
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          <input type="datetime-local" value={dateTimeInput} onChange={(e)=>
           ...
           px-3 text-sm rounded-lg hover:bg-primary cursor-pointer'>Add Time</button>
        </div>
      </div>
      ...
      {Object.keys(dateTimeSelection).length >0 && (
       ...
       <button onClick={handleSubmit} disabled={addingShow} className='bg-primary text-white px-8 py-2
       mt-6 rounded hover:bg-primary/90 transition-all cursor-pointer'>Add Show</button>
    </>
  ):<Loading/>
}
